...
import { ArrowLeft } from 'lucide-react'; // Importing a left arrow icon from the 'lucide-react' library
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await AxiosInstance.post('/api/users/create-payment', {
        amount: checkoutDetails.planPrice * 100, // Razorpay expects amount in paise
        currency: 'INR',
      ...
      if (order.id) {
        const options = {
          ...
          handler: function (response: any) {
            ...
            const res = AxiosInstance.post('/api/users/verify-payment', {
                type: checkoutDetails.planInterval
            })

            
                window.location.href = '/user/premium'  // This will redirect to the /user/premium page

            
            // Handle successful payment
            console.log(res)
            // You should verify the payment on your server here
            ...
            <div className="space-y-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-600 mt-1">Please note that all purchases are final and non-refundable. Make sure you've reviewed your selection before proceeding.</p>
                </div>
              </div>
            </div>
          </CardContent>
